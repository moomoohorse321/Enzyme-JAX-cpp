...
convertLLVMAllocaToMemrefAlloca(FromAlloc alloc, RewriterBase &rewriter,
  ...
  int64_t newElnum;
  if constexpr (!inPlace) {
    auto newElSize = dataLayout.getTypeSize(
        ptr2memref.getResult().getType().getElementType());
    newElnum = elNum / newElSize;
    ...
    if (oldElSize.isScalable())
      return failure();
    auto allocationSize = llvm::checkedMulUnsigned(
    ...
    auto newElSize = dataLayout.getTypeSize(
        ptr2memref.getResult().getType().getElementType());
    if (newElSize.isScalable() || newElSize.isZero())
      return failure();
    uint64_t fixedNewElSize = newElSize.getFixedValue();
    uint64_t newElnumUnsigned = *allocationSize / fixedNewElSize;
    if (*allocationSize % fixedNewElSize != 0 ||
        newElnumUnsigned >
            static_cast<uint64_t>(std::numeric_limits<int64_t>::max()))
      return failure();
    newElnum = static_cast<int64_t>(newElnumUnsigned);
  }
